        ...
        while (change) {
            ...
            for (BasicBlock n : nodes) {
                ...
                if (n == nodes.get(0)) {
                    ...
                    preDom = intersectionBB(domList); // intersection of
                                                      // predecessor's dominator

                    for (BasicBlock tbb : preDom) {
                        // LogConsole.log("\t"+"pre's dom intersection :
                        // "+tbb.getAddress()+"\n");
                    }
        ...
        while (change) {
            ...
            for (int j = nodes.size() - 1; j >= 0; j--) {
                ...
                if (n == nodes.get(nodes.size() - 1)) {
                    ...
                    for (List<BasicBlock> tbl : domList) {
                        // LogConsole.log("\t"+"succ :
                        // "+succ.get(i).getAddress()+"\n");
                        i++;
                        for (BasicBlock tbb : tbl) {
                            // LogConsole.log("\t\t"+"succ's dom :
                            // "+tbb.getAddress()+"\n");
                        }
                    }

                    succDom = intersectionBB(domList); // intersection of
                                                       // predecessor's
                                                       // dominator

                    for (BasicBlock tbb : succDom) {
                        // LogConsole.log("\t"+"succ's dom intersection :
                        // "+tbb.getAddress()+"\n");
                    }
